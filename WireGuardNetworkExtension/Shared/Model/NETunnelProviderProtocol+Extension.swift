    ...
    func asTunnelConfiguration(called name: String? = nil) -> TunnelConfiguration? {
        if let passwordReference = passwordReference,
            ...
            do {
                ...
                log.error("Failed to parse wg-quick config from keychain reference: \(error)")
                return nil
            }
        }
        if let oldConfig = providerConfiguration?["WgQuickConfig"] as? String {
            do {
                ...
                log.error("Failed to parse legacy wg-quick config from providerConfiguration: \(error)")
                return nil
            }
        }
        return nil
