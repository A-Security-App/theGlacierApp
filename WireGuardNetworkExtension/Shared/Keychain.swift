...
class Keychain {
    ...
    static func makeReference(containing value: String, called name: String, previouslyReferencedBy oldRef: Data? = nil) -> Data? {
        ...
        guard var bundleIdentifier = Bundle.main.bundleIdentifier else {
            NSLog("Keychain Unable to determine bundle identifier")
            return nil
        }
        if bundleIdentifier.hasSuffix(".WireGuardNetworkExtension") {
            bundleIdentifier.removeLast(".WireGuardNetworkExtension".count)
        }
