...
private func prettyTimeAgo(timestamp: Date) -> String {
    ...
    case 0: return "Now"
    default:
        return String(format: "%@ ago", prettyTime(secondsLeft: timeInterval))
    }
}
...
private func prettyTime(secondsLeft: Int64) -> String {
    ...
    if years > 0 {
        return years == 1 ? String(format: "tunnelHandshakeTimestampYear (%d)", years) : String(format: "tunnelHandshakeTimestampYears (%d)", years)
    }
    if days > 0 {
        return days == 1 ? String(format: "tunnelHandshakeTimestampDay (%d)", days) : String(format: "tunnelHandshakeTimestampDays (%d)", days)
    }
    if hours > 0 {
        let hhmmss = String(format: "%02d:%02d:%02d", hours, minutes, seconds)
        return String(format: "tunnelHandshakeTimestampHours hh:mm:ss (%@)", hhmmss)
    }
    if minutes > 0 {
        let mmss = String(format: "%02d:%02d", minutes, seconds)
        return String(format: "tunnelHandshakeTimestampMinutes mm:ss (%@)", mmss)
    }
    return seconds == 1 ? String(format: "tunnelHandshakeTimestampSecond (%d)", seconds) : String(format: "tunnelHandshakeTimestampSeconds (%d)", seconds)
}
